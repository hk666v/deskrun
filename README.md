...
| `Alt+Space` | 呼出 / 收起窗口 |
| 直接打字 | 搜索（支持拼音全拼与首字母） |
| `↑` `↓` | 在结果间移动（网格模式下按当前列数跳一整行） |
| `←` `→` | 切换到上一个 / 下一个分组（标签条两头会循环）。光标还在搜索框文字中间时，这两个键先归光标，走到文字两端之后才切分组 |
| `Enter` | 启动选中项；没有匹配时执行列表里的第一条兜底动作 |
...
- **标签条末尾的 `+`** ——就地新建。点一下变成输入框，回车创建，Esc 取消。名字重复时会保留输入并提示，不会丢掉你打的内容
- **设置 → Groups** ——重命名、删除、改顺序都在这里

分组名必须唯一。标签条上并排两个同名标签没法区分，条目编辑器里的分组下拉也选不准，所以这条规则保留。
