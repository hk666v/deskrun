...
**`Alt+Space` 呼出，打字，回车。** Windows 11 上的启动器：手不用离开键盘，桌面上的任何东西两步就能叫出来。

![DeskRun 主界面：左侧是分组栏，右侧是搜索与结果](docs/images/deskrun-list-view.webp)

DeskRun 常驻托盘，按一次全局热键滑出来，光标已经落在搜索框里。程序、文件夹、网址、`cmd` 命令都能搜：支持拼音全拼与首字母，搜索覆盖所有分组，收藏和常用的排在前面，**命中的那段字会被标出来**，所以一眼就知道「为什么这条会被匹配到」。

窗口默认 `760 × 560`，位置和大小记得住，也可以让它每次都出现在鼠标所在那台显示器的正中。关闭窗口只是收起，进程留在托盘里。

## 三步用起来

1. **呼出** —— 按 `Alt+Space`（可以改）。窗口滑出，光标已经在搜索框里，直接开始打字。
2. **打字** —— 名字、备注、拼音全拼、拼音首字母都能搜；`↑` `↓` `Tab` 在结果里走，`←` `→` 在分组栏和结果之间切换。
3. **回车** —— 启动选中的条目。没有匹配时，`Enter` 执行列表里的第一条兜底动作（打开网址、网页搜索、进制转换）。

搜索是这样的：输入「工具」，三个不同分组、不同类型的条目一起命中，命中的字被标出来，条目右边标着它属于哪个分组。

![搜索：输入「工具」](docs/images/deskrun-search.webp)

## 界面

两种显示模式，在 设置 → Display mode 里切换：

- **列表** —— 信息密度高，一行一个条目，适合条目多的人（上面那张主界面就是）
- **网格** —— 卡片更大，图标看得更清楚，适合看备注和参数

![网格视图](docs/images/deskrun-grid-view.webp)

方向键所在的那一侧，选中行是满强度的琥珀洗色 + 选中条；另一侧同样的标记降到四成——就像文件管理器里非活动窗格的选择会变灰。**哪一边是活的，一眼就看得出**。

## 功能

- 全局热键呼出，默认 `Alt+Space`
- 窗口的另外两个键同样可改：`Ctrl+S` 把光标送回搜索框（它就标在搜索框旁边），`Ctrl+O` 收起分组栏。在设置里**点一下那个框、按下要用的组合键**就录好了
- 托盘驻留的应用生命周期，关闭窗口只是隐藏，进程不退出
- 搜索覆盖所有分组与视图；收藏与最近用过的排在前面，同一个条目只出现一次，命中的字会被标出来
- 手动添加启动项：`.exe` / `.lnk`、文件夹、URL、`cmd` 命令
- 把文件或文件夹直接拖进窗口即可导入
- `cmd` 命令型条目支持固定参数、运行时参数、完整命令预览，以及右键菜单里的 `Copy Command`
- 右键 `Duplicate` 复制条目，快速做出参数不同的变体
- Windows 图标提取与图标缓存
- 以管理员身份运行（条目级开关，或右键一次性提权）
- 自定义分组，**分组可嵌套**、可拖拽排序；条目会安静地标出自己在哪个分组（嵌套分组显示完整路径）
- 搜不到时不再只有一句「没有结果」：打开网址、网页搜索、进制转换，回车即用
- 手动扫描已安装的应用（开始菜单 / 桌面 / 注册表），挑想要的导入
- 显示模式：卡片网格 / 紧凑列表
- 界面缩放：`90%` / `100%` / `115%` / `130%`，文字、图标和间距一起变
- 纯本地 JSON 存储；配置目录可自定义，也可整体导出 / 导入
- 很轻：宿主进程闲置约 `9 MB` 私有工作集，整个应用的实际占用见下方[关于占用](#关于占用)
...
| 按键 | 作用 |
| --- | --- |
| `Alt+Space` | 呼出 / 收起窗口。这个键显示在程序名后面（和名字底部对齐），要改去 设置 → Global hotkey：点一下那个框，再按下要用的组合键 |
| 直接打字 | 搜索**所有分组**里的条目（支持拼音全拼与首字母）；收藏与最近用过的排在前面，同一个条目只出现一次 |
| `↑` `↓` | 在**当前所在的面板**里移动：分组栏里是切换视图，结果区里是移动选中项（网格模式下按整行跳） |
| `Tab` `Shift+Tab` | 和 `↓` `↑` 完全一样：只在条目之间往下 / 往上走。它不会把焦点带到搜索框那一排（`+ App`、`Settings` 那些按钮）上去——光标始终留在搜索框里 |
...
| `Ctrl+S` | 把光标送回搜索框——焦点跑到别处（点过按钮、卡片、分组）之后，用它就能立刻接着打字。它就标在搜索框旁边 |
| `Ctrl+O` | 收起 / 展开左侧分组栏。这个键和 `Ctrl+S` 都可以在 设置 → General 里改：点一下那个框，再按下要用的组合键（必须带 Ctrl 或 Alt） |
| `Ctrl+,` | 打开设置 |
| `Esc` | 逐层关闭：菜单 → 对话框 → 设置；都没有时收起窗口。快捷键框正在等按键时，它只取消这一次录制 |
...
![设置面板](docs/images/deskrun-settings.webp)

- **Global hotkey / Focus the search box / Show or hide the groups** —— 三个快捷键都是**按出来的**：点一下框，框里变成「Press the combination」，按下要用的组合键就录进去；`Esc` 取消这一次录制。没有 Ctrl 或 Alt 的组合会被拒绝（单按一个字母会把打字吃掉），框会留在原地等你重按
- **Launch at login / Hide after launch** —— 开机自启，以及启动条目后是否顺手收起窗口
- **Follow the pointer** —— 开：每次都在鼠标所在显示器的正中滑出；关：复用上次的位置
- **Display mode / Interface size** —— 卡片网格还是紧凑列表，以及整体缩放
- **Config folder** —— 配置目录的位置、切换、导出、导入
...
DeskRun 支持通过 `cmd.exe` 执行的命令型启动项。名字给出来，命令和参数分开填：**固定参数**每次都在，**运行时参数**是这次要传的，启动时拼在一起：

![添加 CMD 命令](docs/images/deskrun-command-editor.webp)
...
如果启用了 `Keep CMD window open`，DeskRun 会改用 `/K` 而不是 `/C`。命令预览会原样显示在列表里，右键菜单里的 `Copy Command` 可以直接把整条命令复制走。
...
![兜底动作：输入 0x1f](docs/images/deskrun-fallback.webp)

裸数字按十进制处理，只有 `0x` / `h` 后缀才当作十六进制——`1b` 这种既像二进制的后缀、又像没加前缀的十六进制，一律不猜，直接当作普通搜索词。

### 发现已安装的应用

Discovery 视图里手动跑一次扫描，DeskRun 会走一遍开始菜单、桌面和注册表里登记的已安装程序，把候选列出来，勾选后一次导入：

![发现视图](docs/images/deskrun-discovery-view.webp)

扫描范围可以按需开关（Start Menu / Desktop / Registry），`Hide existing` 会把已经在库里的藏起来，只留下还没收的。**扫描只在你点击时发生**，不会在后台偷偷跑。
...
**每个条目会显示自己在哪个组**：名字右边有一个安静的标签，嵌套的分组显示完整路径（比如 `代理 / 命令行工具`）。在你**正在看的那一组**里不显示——那时候每条都一样，只是噪音。

其余入口：

- **`+ New group`**（栏底）——新建顶层分组。点一下变成输入框，回车创建，Esc 取消；名字重复会保留输入并提示
- **上下拖动分组** ——调整顺序；落在行的中间 = 放进这个分组里，落在线上 = 排在它前/后
- **`Ctrl+O` 或标题行右侧的按钮** ——收起 / 展开整栏，收起后省下的宽度还给条目列表；这个状态会记住
...
说清楚一点：真正属于 DeskRun 的 Rust 宿主进程闲置时占用约 `9 MB` 私有工作集，
...
- 关闭窗口只是隐藏，不会终止进程
- 实际启动在 Rust 侧执行
- 窗口隐藏时（热键、托盘、关闭按钮、启动条目）会把搜索框清空，下一次呼出是干净的
